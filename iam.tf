resource "scalr_iam_team" "team" {
  name        = "alfi-auto_empty_team_${formatdate("DDMMYYYY", timestamp())}"
  description = "Generated by provider on: ${formatdate("DDMMYYYY", timestamp())}"
}

resource "scalr_role" "role" {
  name        = "alfi2-auto_admin_role_${formatdate("DDMMYYYY", timestamp())}"
...
resource "scalr_service_account" "no_permissions_service_acc" {
  name   = "alfi-service-account-${formatdate("DDMMYYYY", timestamp())}"
  status = "Active"
}
